...
**As of 05.01.2021 Scripting is free for all users, it is heavily recommended you scroll down to Zynj's github, check out EWIJSON and learn how to use it. Loading WI with JSONthing alone makes it work like it should.** In the early hours of 04.01.2021 something changed in the AI back-end. `-` `()` became much weaker. We're waiting for further changes from Latitude or for the format enthusiasts to overhaul their formats with discord evidence. Zaltys can still work if you tweak the broken bits out or replace them with different symbols. In the meanwhile remember that simple formats like Caveman are relatively safe from AI-breakage. Most of the information following is for people who want to avoid scripting. Many anecdotal advice parts of this document should now be **dismissed** as **historic** due to the very recent changes.
