...
**Disregard** this section for now and treat the anecdotal parts of this document as historic. In the early hours of 04.01.2021 something changed in the AI back-end. `-` `()` became much weaker and `<>` seems to have stopped working entirely. We're waiting for further changes from Latitude or for the format enthusiasts to overhaul their formats with discord evidence. In the meanwhile remember that simple formats like Caveman are relatively safe from AI-breakage.
